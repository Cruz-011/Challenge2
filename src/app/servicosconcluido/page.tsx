...
import CabecalhoMec from '../../components/cabecalhomec'; // Corrigido para 'components' com 'c' minúsculo
...
interface Servico {
  codigo: string; // Altere o tipo conforme necessário
  nome: string;
  veiculo: string;
  data: string;
  hora: string;
  preco?: number; // Preço pode ser opcional
}
...
const ServicosConcluidos: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedConcluidos) {
      try {
        const parsedConcluidos: Servico[] = JSON.parse(storedConcluidos);
